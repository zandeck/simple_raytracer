...
impl Ray {
    ...
    pub fn color<T>(&self, world: &hitable_list::HitableList<T>, depth: f64) -> V
    where
        T: Hitable,
    {
        let hr = world.hit(self, 0.0, f64::INFINITY);
        match hr {
            Some(h) => match (h.material.scatter(&self, &h), depth < 50.0) {
                (Some(m), true) => m
                    .attenuation
                    .mul_element_wise(m.scattered.color(&world, depth + 1.0)),
